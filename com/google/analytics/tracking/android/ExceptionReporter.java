...
public class ExceptionReporter implements UncaughtExceptionHandler {
    ...
    public ExceptionReporter(Tracker tracker, ServiceManager serviceManager, UncaughtExceptionHandler originalHandler, Context context) {
        if (tracker == null) {
            ...
            Log.m13v("ExceptionReporter created, original handler is " + (originalHandler == null ? "null" : originalHandler.getClass().getName()));
        }
    }
    ...
    public void uncaughtException(Thread t, Throwable e) {
        ...
        Log.m13v("Tracking Exception: " + description);
        this.mTracker.send(MapBuilder.createException(description, Boolean.valueOf(true)).build());
        this.mServiceManager.dispatchLocalHits();
        if (this.mOriginalHandler != null) {
            Log.m13v("Passing exception to original handler.");
