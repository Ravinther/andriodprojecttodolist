...
class SimpleNetworkDispatcher implements Dispatcher {
    ...
    public boolean okToDispatch() {
        ...
        Log.m13v("...no network connectivity");
        return false;
    }
    ...
    public int dispatchHits(List<Hit> hits) {
        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            if (url == null) {
                if (Log.isVerbose()) {
                    Log.m14w("No destination: discarding hit: " + hit.getHitParams());
                } else {
                    Log.m14w("No destination: discarding hit.");
                }
                hitsDispatched++;
            } else {
                String params;
                ...
                if (request == null) {
                    ...
                    if (params.length() > 8192) {
                        Log.m14w("Hit too long (> 8192 bytes)--not sent");
                    } else {
                        if (this.gaInstance.isDryRunEnabled()) {
                            Log.m12i("Dry run enabled. Hit not actually sent.");
                        } else {
                            if (firstSend) {
                                try {
                                    ...
                                    Log.m14w("ClientProtocolException sending hit; discarding hit...");
                                } catch (IOException e2) {
                                    Log.m14w("Exception sending hit: " + e2.getClass().getSimpleName());
                                    Log.m14w(e2.getMessage());
                                }
                            }
                            ...
                            if (statusCode != 200) {
                                Log.m14w("Bad response: " + response.getStatusLine().getStatusCode());
                            }
                        }
                    }
                    hitsDispatched++;
    ...
    private HttpEntityEnclosingRequest buildRequest(String params, String path) {
        ...
        if (full.length() < 2036) {
            ...
            try {
                ...
                Log.m14w("Encoding error, discarding hit");
                return null;
            }
        }
    ...
    private void logDebugInformation(HttpEntityEnclosingRequest request) {
        ...
        if (request.getEntity() != null) {
            try {
                ...
                Log.m13v("Error Writing hit to log...");
            }
        }
        Log.m13v(httpHeaders.toString());
    }
    ...
    URL getUrl(Hit hit) {
        ...
        try {
            ...
            Log.m11e("Error trying to parse the hardcoded host url. This really shouldn't happen.");
            return null;
        }
    }
