...
public final class Fields {
    ...
    public static String customDimension(int index) {
        return getParam(SCREEN_NAME, index);
    }

    public static String customMetric(int index) {
        return getParam(CAMPAIGN_MEDIUM, index);
    }

    public static String contentGroup(int index) {
        return getParam("&cg", index);
    }

    private static String getParam(String prefix, int index) {
        if (index >= 1) {
            return prefix + index;
        }
        Log.m11e("index out of range for " + prefix + " (" + index + ")");
        return "";
    }
}
