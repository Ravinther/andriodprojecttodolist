...
class GAServiceProxy implements ServiceProxy, OnConnectedListener, OnConnectionFailedListener {
    ...
    static /* synthetic */ class C00783 {
        static final /* synthetic */ int[] f20x3c5f147c;

        static {
            f20x3c5f147c = new int[ConnectState.values().length];
            try {
                f20x3c5f147c[ConnectState.CONNECTED_LOCAL.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                f20x3c5f147c[ConnectState.CONNECTED_SERVICE.ordinal()] = GAServiceProxy.MAX_TRIES;
            } catch (NoSuchFieldError e2) {
            }
            try {
                f20x3c5f147c[ConnectState.CONNECTING.ordinal()] = 3;
            } catch (NoSuchFieldError e3) {
            }
            try {
                f20x3c5f147c[ConnectState.PENDING_CONNECTION.ordinal()] = 4;
            } catch (NoSuchFieldError e4) {
            }
            try {
                f20x3c5f147c[ConnectState.PENDING_DISCONNECT.ordinal()] = 5;
            } catch (NoSuchFieldError e5) {
            }
            try {
                f20x3c5f147c[ConnectState.DISCONNECTED.ordinal()] = 6;
    ...
    private class DisconnectCheckTask extends TimerTask {
        ...
        public void run() {
            if (GAServiceProxy.this.state == ConnectState.CONNECTED_SERVICE && GAServiceProxy.this.queue.isEmpty() && GAServiceProxy.this.lastRequestTime + GAServiceProxy.this.idleTimeout < GAServiceProxy.this.clock.currentTimeMillis()) {
                Log.m13v("Disconnecting due to inactivity");
    ...
    public synchronized void setForceLocalDispatch() {
        if (!this.forceLocalDispatch) {
            Log.m13v("setForceLocalDispatch called.");
    ...
    L_0x0028:
        r0 = com.google.analytics.tracking.android.GAServiceProxy.C00783.f20x3c5f147c;	 Catch:{ all -> 0x0072 }
    ...
    L_0x003e:
        ...
        com.google.analytics.tracking.android.Log.m13v(r0);	 Catch:{ all -> 0x0072 }
    ...
    private synchronized void useStore() {
        if (this.state != ConnectState.CONNECTED_LOCAL) {
            clearAllTimers();
            Log.m13v("falling back to local store");
    ...
    private synchronized void connectToService() {
        if (this.forceLocalDispatch || this.client == null || this.state == ConnectState.CONNECTED_LOCAL) {
            Log.m14w("client not initialized.");
            useStore();
        } else {
            try {
                this.connectTries++;
                ...
                Log.m13v("connecting to Analytics service");
                this.client.connect();
            } catch (SecurityException e) {
                Log.m14w("security exception on connectToService");
                useStore();
            }
        }
    }
    ...
    public synchronized void onDisconnected() {
        if (this.state == ConnectState.PENDING_DISCONNECT) {
            Log.m13v("Disconnected from service");
            clearAllTimers();
            this.state = ConnectState.DISCONNECTED;
        } else {
            Log.m13v("Unexpected disconnect.");
    ...
    public synchronized void onConnectionFailed(int errorCode, Intent resolution) {
        this.state = ConnectState.PENDING_CONNECTION;
        if (this.connectTries < MAX_TRIES) {
            Log.m14w("Service unavailable (code=" + errorCode + "), will retry.");
            fireReconnectAttempt();
        } else {
            Log.m14w("Service unavailable (code=" + errorCode + "), using local store.");
            useStore();
        }
    }
