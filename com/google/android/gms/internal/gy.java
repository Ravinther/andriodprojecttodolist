...
public final class gy implements SafeParcelable, Invitation {
    ...
    private void fR() {
        ed.m662v(!this.IF.isEmpty());
        Invitation invitation = (Invitation) this.IF.get(0);
        int size = this.IF.size();
        for (int i = 1; i < size; i++) {
            ed.m660a(invitation.getInviter().equals(((Invitation) this.IF.get(i)).getInviter()), "All the invitations must be from the same inviter");
        }
    }
