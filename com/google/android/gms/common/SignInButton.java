...
public final class SignInButton extends FrameLayout implements OnClickListener {
    ...
    private static Button m132c(Context context, int i, int i2) {
        Button etVar = new et(context);
        etVar.m728a(context.getResources(), i, i2);
        return etVar;
    }

    private void m133v(Context context) {
        if (this.yY != null) {
            removeView(this.yY);
        }
        try {
            this.yY = es.m1825d(context, this.mSize, this.yX);
        } catch (C0194a e) {
            Log.w("SignInButton", "Sign in button not found, using placeholder instead");
            this.yY = m132c(context, this.mSize, this.yX);
        }
        addView(this.yY);
    ...
    public void setStyle(int buttonSize, int colorScheme) {
        ...
        er.m717a(z2, "Unknown button size " + buttonSize);
        if (colorScheme < 0 || colorScheme >= SIZE_ICON_ONLY) {
            z = false;
        }
        er.m717a(z, "Unknown color scheme " + colorScheme);
