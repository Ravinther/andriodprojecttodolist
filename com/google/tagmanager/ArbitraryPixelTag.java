...
class ArbitraryPixelTag extends TrackingTag {
    ...
    class C08091 implements HitSenderProvider {
        final /* synthetic */ Context val$context;

        C08091(Context context) {
    ...
    public ArbitraryPixelTag(Context context) {
        this(context, new C08091(context));
    }
                ...
                if (l instanceof List) {
                    for (Map<Object, Object> m : l) {
                        if (m instanceof Map) {
                            ...
                            Log.m1389e("ArbitraryPixel: additional params contains non-map: not sending partial hit: " + uriBuilder.build().toString());
                            return;
                        }
                    }
                }
                Log.m1389e("ArbitraryPixel: additional params not a list: not sending partial hit: " + uriBuilder.build().toString());
                return;
            }
            String uri = uriBuilder.build().toString();
            this.mHitSenderProvider.get().sendHit(uri);
            Log.m1393v("ArbitraryPixel: url = " + uri);
