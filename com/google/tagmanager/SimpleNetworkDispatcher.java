...
class SimpleNetworkDispatcher implements Dispatcher {
    ...
    public void dispatchHits(List<Hit> hits) {
        ...
        for (int i = 0; i < maxHits; i++) {
            ...
            URL url = getUrl(hit);
            if (url == null) {
                Log.m1395w("No destination: discarding hit.");
                ...
                if (request == null) {
                    ...
                    if (firstSend) {
                        try {
                            ...
                            Log.m1395w("ClientProtocolException sending hit; discarding hit...");
                            this.dispatchListener.onHitPermanentDispatchFailure(hit);
                        } catch (IOException e2) {
                            Log.m1395w("Exception sending hit: " + e2.getClass().getSimpleName());
                            Log.m1395w(e2.getMessage());
                    ...
                    if (statusCode != 200) {
                        Log.m1395w("Bad response: " + response.getStatusLine().getStatusCode());
                        this.dispatchListener.onHitTransientDispatchFailure(hit);
                    } else {
    ...
    private HttpEntityEnclosingRequest constructGtmRequest(URL url) {
        ...
        try {
            ...
            try {
                request2.addHeader("User-Agent", this.userAgent);
                return request2;
            } catch (URISyntaxException e2) {
                e = e2;
                request = request2;
                Log.m1395w("Exception sending hit: " + e.getClass().getSimpleName());
                Log.m1395w(e.getMessage());
                return request;
            }
    ...
    private void logDebugInformation(HttpEntityEnclosingRequest request) {
        ...
        if (request.getEntity() != null) {
            try {
                ...
                Log.m1393v("Error Writing hit to log...");
            }
        }
        Log.m1393v(httpHeaders.toString());
    }
    ...
    URL getUrl(Hit hit) {
        try {
            ...
            Log.m1389e("Error trying to parse the GTM url.");
            return null;
        }
    }
}
