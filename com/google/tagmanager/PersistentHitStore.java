...
class PersistentHitStore implements HitStore {
    ...
    class UrlDatabaseHelper extends SQLiteOpenHelper {
        ...
        private boolean tablePresent(String table, SQLiteDatabase db) {
            ...
            try {
                ...
                Log.m1395w("Error querying for table " + table);
                return false;
            } finally {
                if (cursor != null) {
                    cursor.close();
    ...
    /* renamed from: com.google.tagmanager.PersistentHitStore.1 */
    class C08161 implements Clock {
        C08161() {
        }
    ...
    class StoreDispatchListener implements DispatchListener {
        ...
        public void onHitPermanentDispatchFailure(Hit hit) {
            PersistentHitStore.this.deleteHit(hit.getHitId());
            Log.m1393v("Permanent failure dispatching hitId: " + hit.getHitId());
        }
        ...
        public void onHitTransientDispatchFailure(Hit hit) {
            ...
            if (firstDispatchTime == 0) {
                ...
                PersistentHitStore.this.deleteHit(hit.getHitId());
                Log.m1393v("Giving up on failed hitId: " + hit.getHitId());
            }
        }
    }
    ...
    private void removeOldHitIfFull() {
        ...
        if (hitsOverLimit > 0) {
            ...
            Log.m1393v("Store full, deleting " + hitsToDelete.size() + " hits to make room.");
    ...
    List<String> peekHitIds(int maxHits) {
        ...
        if (maxHits <= 0) {
            Log.m1395w("Invalid maxHits specified. Skipping");
        } else {
            ...
            if (db != null) {
                ...
                try {
                    ...
                    Log.m1395w("Error in peekHits fetching hitIds: " + e.getMessage());
                    if (cursor != null) {
                        cursor.close();
                    }
                } catch (Throwable th) {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    if (cursor.moveToFirst()) {
                        do {
                            if (((SQLiteCursor) cursor).getWindow().getNumRows() > 0) {
                                ...
                                Log.m1395w(String.format("HitString for hitId %d too large.  Hit will be deleted.", new Object[]{Long.valueOf(((Hit) hits2.get(count)).getHitId())}));
                            }
                            count++;
                        } while (cursor.moveToNext());
                    }
                    ...
                    Log.m1395w("Error in peekHits fetching hit url: " + e2.getMessage());
                ...
                hits = hits2;
                try {
                    Log.m1395w("Error in peekHits fetching hitIds: " + e2.getMessage());
                    if (cursor != null) {
                        cursor.close();
                    }
                    return hits;
        ...
        if (hitIds != null && hitIds.length != 0) {
            ...
            if (db != null) {
                try {
                    ...
                    Log.m1395w("Error deleting hits");
                }
            }
        }
    }
    ...
    private void setHitFirstDispatchTime(long hitId, long firstDispatchTime) {
        ...
        if (db != null) {
            ...
            try {
                ...
                Log.m1395w("Error setting HIT_FIRST_DISPATCH_TIME for hitId: " + hitId);
                deleteHit(hitId);
            }
        }
    }
    ...
    int getNumStoredHits() {
        ...
        try {
            ...
            Log.m1395w("Error getting numStoredHits");
            if (cursor != null) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    int getNumStoredUntriedHits() {
        ...
        try {
            ...
            Log.m1395w("Error getting num untried hits");
            if (cursor != null) {
                cursor.close();
            }
        } catch (Throwable th) {
    ...
    public void dispatch() {
        Log.m1393v("GTM Dispatch running...");
        if (this.mDispatcher.okToDispatch()) {
            List<Hit> hits = peekHits(40);
            if (hits.isEmpty()) {
                Log.m1393v("...nothing to dispatch");
    ...
    public void close() {
        try {
            ...
            Log.m1395w("Error opening database for close");
        }
    }
