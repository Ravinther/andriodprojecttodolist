...
class ResourceLoader implements Runnable {
    ...
    private boolean okToLoad() {
        ...
        Log.m1393v("...no network connectivity");
        return false;
    }
    ...
    private void loadResource() {
        if (okToLoad()) {
            Log.m1393v("Start loading resource from network ...");
            ...
            try {
                try {
                    ...
                    Log.m1393v("Successfully loaded resource: " + resource);
                    if (!resource.hasResource()) {
                        Log.m1393v("No change for container: " + this.mContainerId);
                    }
                    this.mCallback.onSuccess(resource.hasResource() ? resource.getResource() : null);
                    Log.m1393v("Load resource from network finished.");
                } catch (IOException e) {
                    Log.m1396w("Error when parsing downloaded resources from url: " + url + " " + e.getMessage(), e);
                    this.mCallback.onFailure(Failure.SERVER_ERROR);
                    networkClient.close();
                }
            } catch (FileNotFoundException e2) {
                Log.m1395w("No data is retrieved from the given url: " + url + ". Make sure container_id: " + this.mContainerId + " is correct.");
                this.mCallback.onFailure(Failure.SERVER_ERROR);
            } catch (IOException e3) {
                Log.m1396w("Error when loading resources from url: " + url + " " + e3.getMessage(), e3);
    ...
    void setCtfeURLPathAndQuery(String urlPathAndQuery) {
        ...
        Log.m1387d("Setting CTFE URL path: " + urlPathAndQuery);
        this.mCtfeUrlPathAndQuery = urlPathAndQuery;
    }

    @VisibleForTesting
    void setPreviousVersion(String version) {
        Log.m1387d("Setting previous container version: " + version);
