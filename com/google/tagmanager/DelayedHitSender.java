...
class DelayedHitSender implements HitSender {
    ...
    public boolean sendHit(String url) {
        if (this.mRateLimiter.tokenAvailable()) {
            if (!(this.mWrapperUrl == null || this.mWrapperQueryParameter == null)) {
                try {
                    ...
                    Log.m1393v("Sending wrapped url hit: " + url);
                } catch (UnsupportedEncodingException e) {
                    Log.m1396w("Error wrapping URL for testing.", e);
                    return false;
                }
            }
        ...
        Log.m1395w("Too many urls sent too quickly with the TagManagerSender, rate limiting invoked.");
        return false;
    }
}
