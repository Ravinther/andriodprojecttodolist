...
final class SwipeProgressBar {
    ...
    void draw(Canvas canvas) {
        ...
        if (this.mRunning || this.mFinishTime > 0) {
            ...
            long iterations = (now - this.mStartTime) / RateLimiter.DEFAULT_MILLISECONDS_PER_TOKEN;
            float rawProgress = ((float) ((now - this.mStartTime) % RateLimiter.DEFAULT_MILLISECONDS_PER_TOKEN)) / 20.0f;
            if (!this.mRunning) {
